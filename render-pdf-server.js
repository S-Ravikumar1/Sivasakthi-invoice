...
app.use((req, _res, next) => {
  if (req.path === "/render" || req.path === "/") {
    console.log(`PDF INCOMING: ${req.method} ${req.path}`);
  }
  next();
});
let browserPromise;
...
app.get("/health",(_,res)=>res.json({ok:true,service:"invoice-pdf"}));
app.post(["/render", "/"],async(req,res)=>{
  const expected = process.env.PDF_API_TOKEN;
  if (expected && req.get("x-pdf-token") !== expected) {
    console.error("PDF AUTH ERROR: invalid or missing PDF_API_TOKEN");
    return res.status(401).json({ error: "Unauthorized" });
  }
  const {invoice={},products=[],subtotal=0,cgst=0,sgst=0,igst=0,roundOff=0,grandTotal=0,taxRate=0}=req.body||{};
  console.log("PDF RENDER: invoice=", invoice.invoiceNo || "unknown");
  ...
  *{box-sizing:border-box}html,body{margin:0;padding:0;background:#fff;font-family:Arial,Helvetica,sans-serif;color:#222}.paper{width:210mm;height:297mm;background:#fff;padding:8mm}.frame{border:2px solid #444;height:281mm;overflow:hidden}.top{height:60px;border-bottom:1px solid #444;display:grid;grid-template-columns:1fr 235px}.title{text-align:center;font-size:18px;font-weight:800;padding-top:10px}.meta{border-left:1px solid #444;text-align:right;padding:5px 8px;font-size:16px;font-weight:800;line-height:24px}.seller{height:105px;text-align:center;border-bottom:2px solid #444;padding:11px 8px;font-size:12px;line-height:18px}.seller .name{font-family:Georgia,serif;font-size:30px;font-weight:700}.party{height:95px;border-bottom:1px solid #444;padding:10px 8px;font-size:12px;line-height:18px}.party .name,.party .addr{font-size:16px;line-height:20px}.party .name{font-weight:800}.table{width:100%;border-collapse:collapse;table-layout:fixed}.table th,.table td{border:1px solid #aaa;padding:7px;font-size:10.5px}.table th{font-size:10px;text-align:center;height:36px}.table .desc{width:55%;text-align:left}.table th:nth-child(2){width:12%}.table th:nth-child(3){width:8%}.table th:nth-child(4){width:12%}.table th:nth-child(5){width:13%}.r{text-align:right}.empty td{height:260px}.summary{display:grid;grid-template-columns:1fr 318px;border-top:1px solid #444}.words{min-height:150px;padding:10px;font-size:9.5px;border-right:1px solid #444}.words b{display:block;margin-bottom:5px}.totals{font-size:10px}.tr{display:flex;justify-content:space-between;padding:7px 8px;border-bottom:1px solid #aaa;min-height:30px}.grand{font-size:12px;font-weight:800}.decl{display:grid;grid-template-columns:1fr 1fr;min-height:145px;border-top:1px solid #444}.half{padding:12px 8px;font-size:10px;position:relative}.half:first-child{border-right:1px solid #444}.auth{position:absolute;left:8px;bottom:15px;font-weight:700}.decl b{display:block;margin-bottom:7px;font-size:10.5px}.decltext{font-size:9.5px;line-height:15px}.pn{font-weight:800;font-size:10.5px;margin-bottom:3px}.ih,.ir{display:grid;grid-template-columns:110px 45px;gap:8px}.ih{font-weight:800;font-size:9px;margin:4px 0 2px;text-transform:uppercase}.ir{font-size:9px;line-height:13px;font-weight:400}.pt{font-weight:700;font-size:9px;margin-top:3px}
  
/* Center item/DC details within Description column */
.description, .desc, td.description { text-align:center !important; }
.description input, .desc input { text-align:center !important; }

/* Slightly thicker visible invoice lines */
.invoice-table, .invoice-table th, .invoice-table td,
.invoice-box, .invoice-section, .totals,
.declaration-signature, .declaration-half, .signature-half {
  border-width: 1.4px !important;
}
.invoice-table { border-collapse: collapse; }
hr { border-top-width: 1.4px !important; }
</style>
<style>
/* Final A4 positioning: content grows naturally and bottom sections stay at page bottom */
html,body{margin:0;padding:0}
@page{size:A4;margin:0}
.sheet,.invoice-page{width:210mm;height:297mm;box-sizing:border-box}
.invoice-page{display:flex;flex-direction:column;min-height:297mm}
.invoice-main{flex:1;display:flex;flex-direction:column;min-height:0}
.products{flex:1;min-height:0}
.products-body{min-height:0}
.items-area{flex:1;min-height:60px}
.totals{flex:none}
.decl{flex:none;min-height:180px}
.declaration-signature{display:grid;grid-template-columns:1fr 1fr;min-height:180px}
.declaration-signature .declaration-half{grid-column:1}
.declaration-signature .signature-half{grid-column:2}
</style>
</head><body><div class="paper"><div class="frame"><div class="top"><div class="title">TAX INVOICE</div><div class="meta"><div>INVOICE NO: <strong>${esc(invoice.invoiceNo||"")}</strong></div><div>DATE: <strong>${esc(invoice.date||"")}</strong></div></div></div><div class="seller"><div class="name">${esc(invoice.companyName||"YOUR COMPANY NAME")}</div><div>${esc(invoice.companyAddress||"")}</div><div>${esc(invoice.phone||"")}${invoice.phone&&invoice.email?" • ":""}${esc(invoice.email||"")}</div><div>GSTIN: ${esc(invoice.gstin||"")}</div></div><div class="party"><b>PARTY'S NAME:</b><div class="name">${esc(invoice.partyName||"")}</div><div class="addr">${esc(invoice.partyAddress||"")}</div><div>GSTIN: ${esc(invoice.partyGstin||"—")}</div></div><table class="table"><thead><tr><th class="desc">Particular (Description & Specification)</th><th>HSN Code</th><th>Qty</th><th>Rate</th><th>Amount</th></tr></thead><tbody>${rows}<tr class="empty"><td></td><td></td><td></td><td></td><td></td></tr></tbody></table><div class="summary"><div class="words"><b>Total Amount (Rs. in Words):</b><div>${words(grandTotal)} Rupees Only</div></div><div class="totals"><div class="tr"><span>Total</span><b>${money(subtotal)}</b></div>${taxRows}<div class="tr"><span>Round Off</span><span>${money(roundOff)}</span></div><div class="tr grand"><span>Grand Total</span><b>${money(grandTotal)}</b></div></div></div><div class="decl"><div class="half"><b>Declaration</b><div class="decltext">I declare that this invoice shows the actual price of the jobwork and all the Description are true and correct to the best of my knowledge.</div></div><div class="half"><b>For ${esc(invoice.companyName||"")}</b><div class="auth">Authorized Signatory</div></div></div></div></div></body></html>`;
  let browser; try { browser=await getBrowser(); const page=await browser.newPage(); await page.setContent(html,{waitUntil:"networkidle0"}); const pdf=await page.pdf({format:"A4",printBackground:true,margin:{top:"0mm",right:"0mm",bottom:"0mm",left:"0mm"}}); await page.close(); console.log("PDF SUCCESS: invoice=", invoice.invoiceNo || "unknown", "bytes=", pdf.length); res.setHeader("Content-Type","application/pdf"); res.setHeader("Content-Disposition",`attachment; filename="${String(invoice.invoiceNo||"tax-invoice").replace(/[^a-zA-Z0-9_-]/g,"_")}.pdf"`); return res.send(pdf); } catch(e){ console.error("PDF RENDER ERROR:", e); return res.status(500).json({error:"PDF generation failed",detail:String(e?.message||e).slice(0,300)}); }
});
const port=process.env.PORT||10000;app.listen(port,"0.0.0.0",()=>console.log(`PDF service listening on ${port}`));
